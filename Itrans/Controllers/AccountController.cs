...
namespace Itrans.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    { var user = await UserManager.FindAsync(model.Email, model.Password);
                        if (user.LockoutEnabled) { LogOff(); return View("Lockout"); }
                    }
        ...
        public ActionResult Delete1(string[] Ids,string action)
        {
            ViewBag.Id = Ids;
            if (action == "ban")
            {
        ...
        public async Task<ActionResult> BanConfirmed(string[] UserId)
        {
            foreach (var i in UserId)
                {
                        IdentityResult result = await UserManager.SetLockoutEnabledAsync(i, true);
                }
            return RedirectToAction("Index", "Home");
        }
